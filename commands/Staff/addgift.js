...
module.exports.run = async (bot, message, args) => {
    ...
    if (!bot.config.devs.includes(message.author.id)) return message.channel.createMessage("Sorry but you can not use this command.")
    userID = args[0]
    ...
    if (!user[0]) return message.channel.createMessage("Invalid User ID")
    user = user[0]
